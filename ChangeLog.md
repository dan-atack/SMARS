...
20. BUGFIX: Keep track of the amount of structures that have been demolished, or otherwise drop the imported structure ID's for loaded buildings, as the current system is leading to the creation of duplicate module/floor/connector serial numbers, assigned since the Infra class's internal serial number counter is behind some of the serials for loaded structures if they are from a file in which some modules have been demolished. Good sleuthing there to identify the cause of the problem, at least! ADDENDUM: A solution was implemented, based on existing code from the connectors placement logic, which solves the problem by essentially skipping the serial counter ahead every time a saved ID is loaded. This causes some gaps in the serial number series, but this is acceptable as it solves the problem here, which was the creation of duplicate serial numbers. This way there are some gaps, yes, but at least we can be sure that every structure's ID number is unique.
...
9. Add a new Minimap method, called updateScreenPosition, which will be called by the Engine's scroll handler whenever the player scrolls past a column. Start by having it print the numbers of the grid reference for both edges of the screen (rounded down to the nearest integer if the screen's edge is not exactly flush with the terrain grid).

10. Add a new field to the Minimap class called screenPosition, which will have as its value the index number of the leftmost map column that is fully visible on the main screen. Use this value to determine the position of the 'current screen' rectangle, and link it to the updateScreenPosition method to allow Engine scrolling to move it around. The result of this work should be an accurate location indicator for the Minimap.
...
Not explicitly addressed in this chapter, but worth considering all the same is the issue of tagging images with version data so as to keep track of images as the game's development progresses. It may be worth adding some sort of a top-level metadata file and pulling the version info from that to the frontend's constants file once such a process is established.

Exit Criteria:

- Docker Hub repo is set up to store the game's Docker image files
- Image can be pulled from Docker Hub to any of the game's AWS environments (dev, staging or production)
...
- A new script is developed for the cloud environment, which pulls the latest Docker image from the Docker Hub repo and uses it to update the currently-running Docker compose stack with as little disruption time as possible
