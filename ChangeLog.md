...
- The game's server/backend runs in a container generated by a Dockerfile
- The game's frontend runs in a container generated by a Dockerfile
- When run in alongside a mongo db container (generated with a `docker run` command), the game works as it did in the previous chapter
- The backend container must be able to communicate with the database container
- The frontend container must be able to communicate with the backend container
...
### 5. Make a Dockerfile for the frontend, that installs the necessary packages and so forth on the Ubuntu OS, and then runs the game's development server (more on this to come later in the chapter... or in a later chapter?)

## Chapter Three: Docker Compose
